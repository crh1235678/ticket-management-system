...
public class FlightStrategy implements TransportStrategy {
    ...
    public IPage<OptionalTicketVO> changeTickets(ChangeTicketDTO dto,Order order){
        ...
        // 排除原票
        wrapper.ne(Flight::getId, order.getTicketId());
        // 剩余座位数需要大于用户购买数
        wrapper.ge(Flight::getSeatRemaining, order.getSeatCount());
        // 限制还未发车
        wrapper.gt(Flight::getDepartureTime, LocalDateTime.now());
        wrapper.orderByAsc(Flight::getDepartureTime);

        Page<Flight> page = new Page<>(dto.getPageNum(), dto.getPageSize());
        IPage<Flight> flightIPage = flightMapper.selectPage(page, wrapper);

        System.out.println("看这里1" + flightIPage);
        return flightIPage.convert(flight -> {
            OptionalTicketVO vo = new OptionalTicketVO();
            vo.setTransportType(TransportType.FLIGHT);
            vo.setSeatCount(order.getSeatCount());
            vo.setTransportId(flight.getId());
            vo.setNumber(flight.getFlightNumber());
            ...
            vo.setSeatRemaining(flight.getSeatRemaining());
            BigDecimal newPrice = flight.getPrice()
    ...
    public List<Long> queryIds(QueryDTO query) {
        ...
        wrapper.like(StringUtils.isNotBlank(query.getNumber()),
                Flight::getFlightNumber, query.getNumber());

        // 出发地
        wrapper.like(StringUtils.isNotBlank(query.getOrigin()),
                Flight::getOrigin, query.getOrigin());

        // 目的地
        wrapper.like(StringUtils.isNotBlank(query.getDestination()),
                Flight::getDestination, query.getDestination());
        ...
        // 查询航班ID
        return flightMapper.selectList(wrapper)
                .stream()
                .map(Flight::getId)
    ...
    public void buildCache(List<Order> orders, TransportCache cache) {
        ...
        List<Long> ids = orders.stream()
                .filter(o -> TransportType.FLIGHT.equals(o.getTransportType()))
                .map(Order::getTicketId)
                .distinct()
                .toList();

        if (!ids.isEmpty()) {
            Map<Long, Flight> map = flightService.listByIds(ids)
                    .stream()
                    .collect(Collectors.toMap(Flight::getId, f -> f));

            cache.setFlightMap(map);
        }
    }
    ...
    public void fillTransportInfo(OrderUserVO vo, Order order, TransportCache cache) {
        ...
        Flight flight = cache.getFlightMap().get(order.getTicketId());

        if (flight != null) {
            vo.setTicketId(flight.getId());
            vo.setNumber(flight.getFlightNumber());
            vo.setName(flight.getFlightName());
            vo.setOrigin(flight.getOrigin());
            vo.setDestination(flight.getDestination());
            vo.setDepartureTime(flight.getDepartureTime());
            vo.setArrivalTime(flight.getArrivalTime());
    ...
    public Integer reduceSeat(Long id, Integer count, Integer version) {
        return flightMapper.reduceSeat(id, count, version);
    }

    // 增加座位
    @Override
    public Integer addSeat(Long id, Integer count, Integer version) {
        return flightMapper.addSeat(id, count, version);
    }
