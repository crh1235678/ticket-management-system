<template>
  <div class="bus-container">
    ...
    <!-- 班车表 -->
    <el-table :data="busList" border width="100%">

      <!-- 班车信息 -->
      <el-table-column label="班车">
        <template #default="scope">
          <div class="bus-info">
            <div>
              <div>{{ scope.row.busName }}</div>
              <div class="sub">{{ scope.row.busNumber }}</div>
            </div>
          </div>
        </template>
      </el-table-column>
...
import type { BusUserVO } from '@/types/bus'
import PurchaseDialog from '@/components/OrderDialog.vue'
import PaymentDialog from '@/components/PaymentDialog.vue'

const busList = ref<BusUserVO[]>([])
...
async function getList() {
  ...
  const res = await busApi.getlist(constants.PAGE_NUM, constants.PAGE_SIZE, params)
  busList.value = res.data.records
}
